...
function DepthScalingContent(props: { recurrentDepthResults: any; config: any; latentAccuracy: number; formatPercent: any }) {
  const { recurrentDepthResults, config, latentAccuracy, formatPercent } = props;
  if (!recurrentDepthResults || recurrentDepthResults.length === 0) {
    return null;
  }
  return React.createElement(React.Fragment, null,
    React.createElement(AccuracyVsDepthChart, {
      data: recurrentDepthResults.map((r: any) => ({
        step: r.step,
        accuracy: r.accuracy,
    ...
    React.createElement('div', { className: "grid md:grid-cols-3 gap-4 mt-6" },
      ...
      React.createElement(MetricCard, {
        ...
        value: formatPercent(Math.max(...recurrentDepthResults.map((r: any) => r.accuracy)) - (recurrentDepthResults[0]?.accuracy || 0)),
        evidenceBadge: "LIVE_TOY_COMPUTATION",
      })
    ),
...
function LatentsPageContent(props: { config: any; stateTrajectory: any; stateChanges: any; latentAccuracy: number; latentStepsUsed: number; showFullTrajectory: boolean; trajectoryTitle: string; trajectoryWidth: number; trajectoryCellSize: number; setShowFullTrajectory: any; setConfig: any; recurrentDepthResults: any; formatPercent: any }) {
  const { config, stateTrajectory, stateChanges, latentAccuracy, latentStepsUsed, showFullTrajectory, trajectoryTitle, trajectoryWidth, trajectoryCellSize, setShowFullTrajectory, setConfig, recurrentDepthResults, formatPercent } = props;
  return React.createElement(React.Fragment, null,
    ...
    React.createElement(EvidencePanel, {
      ...
      children: React.createElement(React.Fragment, null,
        ...
        React.createElement(DepthScalingContent, { recurrentDepthResults, config, latentAccuracy, formatPercent })
      )
    }),
    React.createElement(EvidencePanel, {
      ...
      children: React.createElement('div', { className: "max-w-none" },
...
export default function LatentsPage() {
  ...
  return React.createElement(PageLayout, {
    title: "Think in Latents",
    description: "Recurrent latent reasoning where computation happens through repeated hidden state updates without generating visible tokens.",
    children: React.createElement(LatentsPageContent, {
      config,
      stateTrajectory,
      stateChanges,
      latentAccuracy,
